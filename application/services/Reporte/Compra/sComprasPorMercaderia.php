<?php
  ...
  class sComprasPorMercaderia extends MY_Service {

          public $ComprasPorMercaderia = array();
          ...
          function IndicarValorProducto($data)
          {
            if ($data['IdProducto_Mercaderia'] == '0')
            {
              return $data['IdProducto_Mercaderia'] = '%';
            }
            else
            {
              if ($data['IdProducto_Mercaderia'] == '1')
              {
                return $data['IdProducto_Mercaderia'] = $data['TextoMercaderia_Mercaderia'];
              }
              else
              {
          ...
          {
            $data["FechaInicial"] = convertToDate($data["FechaInicio_Mercaderia"]);
            $data["FechaFinal"] = convertToDate($data["FechaFinal_Mercaderia"]);
            $data['IdProducto'] = $this->IndicarValorProducto($data);
            $data["NombreArchivoReporte"] = $data["NombreArchivoReporte_Mercaderia"];
            $data["NombreArchivoJasper"] = $data["NombreArchivoJasper_Mercaderia"];
          ...
          function ObtenerNombreArchivoReporte()
          {
            ...
            $data['IdParametroSistema']= NOMBRE_ARCHIVO_REPORTE_COMPRAS_POR_MERCADERIAS;
          ...
          {
            $data['IdParametroSistema']= NOMBRE_ARCHIVO_JASPER_COMPRAS_POR_MERCADERIA;
            $resultado = $this->sParametroSistema->ObtenerParametroSistemaPorId($data);
            if (is_string($resultado))
            {
              return $resultado;
