...
class sProductosPorFamiliaConsolidado extends MY_Service {

  public $ProductosPorFamiliaConsolidado = array();
  ...
  function GenerarReportePDF($data) {
    ...
    $data["Vendedores"] = ($data["Vendedores"] > 0) ? "'" . implode("','", $data["Vendedores"]) . "'" : "";
  ...
  function ObtenerNombreArchivoReporte()
  {
    ...
    $data['IdParametroSistema']= NOMBRE_ARCHIVO_REPORTE_PRODUCTOS_POR_FAMILIA_CONSOLIDADO;
  ...
  function ObtenerNombreArchivoJasper()
  {
    ...
    $data['IdParametroSistema']= NOMBRE_ARCHIVO_JASPER_PRODUCTOS_POR_FAMILIA_CONSOLIDADO;
    $resultado = $this->sParametroSistema->ObtenerParametroSistemaPorId($data);
    if (is_string($resultado))
    {
      return $resultado;
