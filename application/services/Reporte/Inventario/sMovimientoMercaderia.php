<?php
  ...
  class sMovimientoMercaderia extends MY_Service {
          ...
          function GenerarReportePDF($data)
          {
            ...
            if ($data['Saldos'] == 1) {
              $resultadoalamcen= $this->sMovimientoAlmacen->CalcularCantidadesProductoPorMovimientoAlmacen($data, false);
            }
          ...
          function ObtenerNombreArchivoReporte()
          {
            ...
            $data['IdParametroSistema']= NOMBRE_ARCHIVO_REPORTE_MOVIMIENTO_ALMACEN;
          ...
          {
            $data['IdParametroSistema']= NOMBRE_ARCHIVO_JASPER_MOVIMIENTO_ALMACEN;
            $resultado = $this->sParametroSistema->ObtenerParametroSistemaPorId($data);
            if (is_string($resultado))
            {
              return $resultado;
