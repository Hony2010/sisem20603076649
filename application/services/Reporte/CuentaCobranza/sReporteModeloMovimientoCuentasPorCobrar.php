...
class sReporteModeloMovimientoCuentasPorCobrar extends MY_Service {
  ...
  function GenerarReportePDF($data) {
    ...
    $data["Vendedores"] = ($data["VendedoresSeleccionados"] > 0) ? "'".implode("','", $data["VendedoresSeleccionados"])."'" : "";
  ...
  function ObtenerNombreArchivoReporte()
  {
    ...
    $data['IdParametroSistema']= NOMBRE_ARCHIVO_REPORTE_MODELO_MOVIMIENTO_CUENTAS_POR_COBRAR;
  ...
  {
    $data['IdParametroSistema']= NOMBRE_ARCHIVO_JASPER_MODELO_MOVIMIENTO_CUENTAS_POR_COBRAR;
    $resultado = $this->sParametroSistema->ObtenerParametroSistemaPorId($data);
    if (is_string($resultado))
    {
      return $resultado;
