...
class cNotaDebito extends CI_Controller  {
	...
	public function ConsultarComprobantesVentaPorCliente()
	{
		...
		$output["resultado"] = $this->sComprobanteVenta->ConsultarComprobantesVentaPorClienteParaDebito($input);
	...
	public function ConsultarNotaDebitoPorIdProducto()
	{
		$data["IdProducto"] = $this->input->post("Data");
		$resultado = $this->sNotaDebito->ConsultarNotaDebitoPorIdProducto($data);
		echo $this->json->json_response($resultado);
	}

	public function ConsultarSugerenciaNotaDebitoPorNombreProducto($data)
	{
		$q =$data;// $this->input->post("Data");
		$data["textofiltro"] = $q;
		$resultado = $this->sNotaDebito->ConsultarSugerenciaNotaDebitoPorNombreProducto($data, 1);
		echo $this->json->json_response($resultado);
	}
	...
	public function InsertarNotaDebito()
	{
		try {
			$this->db->trans_begin();
			...
			$resultado = ($this->ParametroCaja == 1) ? $this->sCajaVenta->InsertarVentaConCaja($data) : $this->sNotaDebito->InsertarNotaDebito($data);
			if(is_array($resultado)) {
				$resultado["CodigoMotivoNotaDebito"] = $data["CodigoMotivoNotaDebito"];
				$resultado["NombreMotivoNotaDebito"] = $data["NombreMotivoNotaDebito"];
				$resultado["MotivoNotaDebito"] = $data["MotivoNotaDebito"];

				$CodigoSerie=substr($data["SerieDocumento"], 0,1);
				if ($CodigoSerie == 'F' || $CodigoSerie == 'B') {
					$resultado2=$this->sNotaDebito->GenerarXMLNotaDebito($resultado);
	...
	public function ActualizarNotaDebito()
	{
		try {
			$this->db->trans_begin();
			$data = $this->input->post("Data");
			...
			$resultado = ($this->ParametroCaja == 1) ? $this->sCajaVenta->ActualizarVentaConCaja($data) : $this->sNotaDebito->ActualizarNotaDebito($data);
			if(is_array($resultado)) {
				$resultado["CodigoMotivoNotaDebito"] = $data["CodigoMotivoNotaDebito"];
				$resultado["NombreMotivoNotaDebito"] = $data["NombreMotivoNotaDebito"];
				$resultado["MotivoNotaDebito"] = $data["MotivoNotaDebito"];
	...
	public function BorrarNotaDebito()
	{
		$data = $this->input->post("Data");
		$resultado = $this->sNotaDebito->BorrarNotaDebito($data);
		echo $this->json->json_response($resultado);
	}

	public function ObtenerNotaDebitoPorIdProducto()
	{
		$data = $this->input->post("Data");
		$resultado = $this->sNotaDebito->ObtenerNotaDebitoPorIdProducto($data);
		echo $this->json->json_response($resultado);
	}
