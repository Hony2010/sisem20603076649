...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:include href="C:/xampp/htdocs/sisem/assets/data/facturacionelectronica/templates/commons/error/error_utils.xsl" dp:ignore-multiple="yes" />
  ...
  <xsl:template match="/*">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:if test='not(matches(./cac:AccountingSupplierParty/cbc:CustomerAssignedAccountID,"^[0-9]{11}$"))'> <!-- regexp:match -->
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:if test='not(matches(./cbc:ID,"[R][C]-[0-9]{8}-[0-9]{1,5}"))'><!-- regexp:match -->
    ...
    <xsl:if test='not(matches(./cac:AccountingSupplierParty/cbc:AdditionalAccountID,"^[6]{1}$"))'><!-- regexp:match -->
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:if test='not(matches(./cac:AccountingSupplierParty/cac:Party/cac:PartyLegalEntity/cbc:RegistrationName,"^[^\s].{1,100}"))'><!-- regexp:match -->
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:if test='not(matches(./cbc:ReferenceDate,"^[0-9]{4}-[0-9]{2}-[0-9]{2}$"))'><!-- regexp:match -->
  ...
  -->
	...
	<xsl:for-each select="sac:SummaryDocumentsLine">
      ...
      <xsl:choose>
        <xsl:when test="not(matches(./cbc:LineID,'[0-9]{1,}'))" ><!-- regexp:match -->
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:if test='not(matches(./sac:DocumentSerialID,"[B][A-Z0-9]{3}"))'><!-- regexp:match -->
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:if test='not(matches(./sac:StartDocumentNumberID,"[0-9]{1,8}"))'><!-- regexp:match -->
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:if test='not(matches(./sac:EndDocumentNumberID,"[0-9]{1,8}"))'><!-- regexp:match -->
      ...
      <xsl:if test='not(matches(./sac:TotalAmount,"^[0-9]{1,12}(\.[0-9]{1,2})?$"))'><!-- regexp:match -->
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:for-each select="./sac:BillingPayment">
            ...
            <xsl:if test='not(matches(./cbc:PaidAmount,"^[0-9]{1,12}(\.[0-9]{1,2})?$"))'><!-- regexp:match -->
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:for-each select="./cac:AllowanceCharge">
            ...
            <xsl:if test='not(matches(./cbc:Amount,"^[0-9]{1,12}(\.[0-9]{1,2})?$"))'>
        ...
        <xsl:otherwise>
          <xsl:for-each select="./cac:TaxTotal">
            <xsl:if test="not(string(./cbc:TaxAmount))">
    	      ...
    	      <xsl:if test='not(matches(./cbc:TaxAmount,"^[0-9]{1,12}(\.[0-9]{1,2})?$"))'> <!-- regexp:match -->
            ...
            <xsl:if test="not(matches(./cac:TaxSubtotal/cbc:TaxAmount,'^[0-9]{1,12}(\.[0-9]{1,2})?$'))"><!-- regexp:match -->
            ...
            <xsl:if test="not(matches(./cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID,'[0-9]{4}'))"><!-- regexp:match -->
    ...
    <func:function name="gemfunc:is-blank">
		...
		<func:result select="matches($data,'^[\s]*$')"/><!-- regexp:match -->
	</func:function>
