...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:include href="C:/xampp/htdocs/sisem/assets/data/facturacionelectronica/templates/commons/error/validate_utils.xsl" dp:ignore-multiple="yes" />
	...
	<xsl:template match="cac:BillingReference">
		...
		<xsl:variable name="tipoDocumentoReferencia" select="cac:InvoiceDocumentReference/cbc:DocumentTypeCode"/>
		<xsl:variable name="serieNumeroDocumentoReferencia" select="cac:InvoiceDocumentReference/cbc:ID"/>
		...
		<xsl:call-template name="existAndRegexpValidateElement">
			<xsl:with-param name="errorCodeNotExist" select="'2512'"/>
			<xsl:with-param name="errorCodeValidate" select="'2513'"/>
			<xsl:with-param name="node" select="$tipoDocumentoReferencia"/>
		...
		SI tipo de comprobante es  07 ó 08 validar:
		 ...
		 -->
		<xsl:if test="gemfunc:is-blank($serieNumeroDocumentoReferencia)">
			<xsl:call-template name="rejectCall">
