...
<#list listaReferencia as referencia>
  <cac:BillingReference>
    <cac:InvoiceDocumentReference>
      <cbc:ID>${referencia.numDocAfectado}</cbc:ID>
      <cbc:IssueDate>${referencia.fecDocAfectado}</cbc:IssueDate>
      <cbc:DocumentTypeCode>${referencia.tipDocAfectado}</cbc:DocumentTypeCode>
      <cbc:DocumentType><![CDATA[ FACTURA ELECTRONICA BASICA ]]></cbc:DocumentType>
    </cac:InvoiceDocumentReference>
  </cac:BillingReference>
</#list>
...
<#list listaTributos as tributo>
  <#if (tributo.mtoBaseImponible?eval gt 0 || tributo.mtoTributo?eval gte 0) >
  ...
  <cac:LegalMonetaryTotal>
    <cbc:LineExtensionAmount currencyID="${moneda}">${TotalMontoBaseImponible}</cbc:LineExtensionAmount>
...
<#if (detalleAdicional.codTipoVariable = "47" || detalleAdicional.codTipoVariable = "48") >
    ...
    <cac:TaxTotal>
      ...
      <#if (detalle.codTriIGV?? && detalle.codTriIGV != "-" && (detalle.mtoBaseIgvItem?eval gt 0 || detalle.mtoBaseIgvItemReferencial?eval gt 0 || detalle.mtoIgvItem?eval gte 0))>
        <#if (detalle.codTriIGV = "1000" || detalle.codTriIGV = "1016" || detalle.codTriIGV = "9995" || detalle.codTriIGV = "9996" || detalle.codTriIGV = "9998" || detalle.codTriIGV = "9997")>
          <cac:TaxSubtotal>
            <#assign pvUnitario = detalle.mtoPrecioVentaUnitario?number>
            <#if (pvUnitario?number gt 0) || (detalle.mtoIgvItem?eval gte 0) >
