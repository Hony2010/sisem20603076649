		...
		init: function() {
			...
			if( this.settings.autoButtons ) {
				this.$steps.each(function(i, el) {
					...
					if( i > 0 ) {
						$content.append('<a href="#" class="'+mthis.settings.autoButtonsPrevClass+'" data-acc-btn-prev>Anterior</a>');
					}
					...
					if( i < ( mthis.$steps.length - 1 ) ) {
						...
						$content.append('<button class="' + mthis.settings.autoButtonsNextClass + '">' + btnText + '</button>');
					}
