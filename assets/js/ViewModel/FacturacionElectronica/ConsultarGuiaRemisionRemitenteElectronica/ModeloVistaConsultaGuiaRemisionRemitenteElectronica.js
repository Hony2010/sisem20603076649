ConsultasGuiaRemisionRemitenteElectronicaModel = function (data) {
    var self = this;
    ...
    self.GenerarPDF = function(data, event){
      if(event)
      {
        ...
        $.ajax({
          ...
          url: SITE_URL+'/FacturacionElectronica/cGuiaRemisionRemitenteElectronica/ExportarPDF',
    ...
    self.MostrarXML = function(data, event){
      if(event)
      {
        ...
        $.ajax({
          ...
          dataType: "html",
          context: document.body,
          url: SITE_URL+'/FacturacionElectronica/cGuiaRemisionRemitenteElectronica/MostrarXML',
          success: function (data) {
              console.log(data);
              if(data != "")
              {
    ...
    self.DescargarXML = function(data, event){
      if(event)
      {
        ...
        $.ajax({
          ...
          dataType: "json",
          context: document.body,
          url: SITE_URL+'/FacturacionElectronica/cGuiaRemisionRemitenteElectronica/DescargarXML',
          success: function (data) {
              console.log(data);
              if(data != "")
              {
    ...
    self.ObtenerFilaClienteJSON = function (data, event) {
      if (event) {
        codigo = data.IdDestinatario;//data.IdPersona;
    ...
    self.EnviarEmailCliente = function(data, event)
    {
      if(event) {
        ...
        if(data_objeto.SerieDocumento.search(CODIGO_SERIE_GUIA) >= 0 && data_objeto.IdTipoDocumento == ID_TIPO_DOCUMENTO_GUIA) {
              ...
              if(rpta.Email == null || rpta.Email == "") {
                ...
                LoadNotificacionEmail(data_notify, function(res){
                  if(res)
                  {
                    ...
                    rpta.IdGuiaRemisionRemitente = data_objeto.IdGuiaRemisionRemitente;
    ...
    self.EnviarMail = function(data,event,callback) {
      if(event) {
        ...
        $.ajax({
          ...
          data : datajs,
          dataType : "json",
          url: SITE_URL+'/Venta/cGuiaRemisionRemitente/EnviarEmail',
          success: function (data) {
            callback(data,event);
          },
          error : function (jqXHR, textStatus, errorThrown) {
...
BuscadorModel = function (data) {
    ...
    self.LimpiarOpciones = function(event){
        if(event)
       {
         ...
         Models.data.ConsultaGuiaRemisionRemitenteElectronica([]);
       }
    ...
    self.BuscarFactura =function(data,event){
      if(event)
      {
        ...
        $.ajax({
          ...
          data : datajs,
          dataType: "json",
          url: SITE_URL+'/FacturacionElectronica/cConsultaGuiaRemisionRemitenteElectronica/ConsultarGuiasRemisionRemitenteElectronica',
          success: function (data) {
              console.log(data);
              if(data != "")
              {
                Models.data.ConsultasGuiaRemisionRemitenteElectronica.removeAll();

                Models.data.ConsultasGuiaRemisionRemitenteElectronica([]);
                cantidad_filas = 0;
                ko.utils.arrayFirst(data, function(item) {
                    if(item.IndicadorEstadoCPE == ESTADO_CPE.GENERADO){
                      cantidad_filas++;
                    }
                    Models.data.ConsultasGuiaRemisionRemitenteElectronica.push(new ConsultasGuiaRemisionRemitenteElectronicaModel(item));
                });
...
var MappingConsultaGuiaRemisionRemitenteElectronica = {
    'ConsultasGuiaRemisionRemitenteElectronica': {
        create: function (options) {
            if (options)
              return new ConsultasGuiaRemisionRemitenteElectronicaModel(options.data);
            }
    },
    'ConsultaGuiaRemisionRemitenteElectronica': {
        create: function (options) {
            if (options)
              return new ConsultasGuiaRemisionRemitenteElectronicaModel(options.data);
            }
