VistaModeloTransportista = function (data) {
    ...
    self.OnNuevo = function (data,event,callback) {
      if (event) {
        $('#formtransportista').resetearValidaciones();
        if(callback) self.callback = callback;
        self.NuevoTransportista(data,event);
    ...
    self.OnEditar = function (data,event,callback) {
      if(event) {
          if (self.IndicadorReseteoFormulario === true)  $('#formtransportista').resetearValidaciones();
          if(callback) self.callback = callback;
          self.EditarTransportista(data,event);
          self.InicializarVistaModelo(data,event);
    ...
    self.OnEliminar = function(data,event,callback) {
      if(event) {
          self.EliminarTransportista(data,event,callback);
      }
    }
    ...
    self.OnClickBtnGrabar = function(data, event)  {
      if(event) {
        if($("#formtransportista").isValid() === false) {
          ...
          //alertify.confirm(self.titulo,"¿Desea guardar los cambios?",function() {
            $("#loader").show();
            self.GuardarTransportista(event,function($data,$event){
              if($data.error) {
                $("#loader").hide();
    ...
    self.OnClickBtnDeshacer = function(data,event)  {
      if(event) {
        self.OnEditar(self.TransportistaInicial,event,self.callback);
      }
    }

    self.OnClickBtnLimpiar = function (data,event) {
      if(event) {
        self.OnNuevo(self.TransportistaInicial,event,self.callback);
      }
    }
      ...
      if (event) {
        //if(!self.procesado) {
          ...
          $("#NumeroDocumentoIdentidad").validate(function(valid, elem) {
            ...
            if(valid === true) {
              $("#loader").show();
              self.ObtenerTransportistaPorServicioExterno(data,event,function($data,$event) {
    ...
    self.Show = function(event) {
      if(event) {
        self.showTransportista(true);
      }
    }

    self.Hide = function(event) {
      if(event) {
        self.showTransportista(false);//$("#modalTransportista").modal("hide");
    ...
    self.MostrarTitulo = ko.computed( function () {
      if (self.opcionProceso() == opcionProceso.Nuevo)  {
        self.titulo = "REGISTRO DE TRANSPORTISTA";
      }
      else {
        self.titulo = "EDICIÓN DE TRANSPORTISTA";
      }
