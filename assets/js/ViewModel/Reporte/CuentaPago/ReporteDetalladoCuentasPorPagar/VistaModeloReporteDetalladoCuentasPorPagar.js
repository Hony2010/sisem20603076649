VistaModeloReporteDetalladoCuentasPorPagar = function (data) {
  var self = this;
  ko.mapping.fromJS(data, {}, self);

  self.RazonSocial = ko.observable('');
  self.NumeroDocumentoIdentidad = ko.observable('');
  self.IdForm = "#FormReporteDetalladoCuentasPorPagar";
  self.form = $(self.IdForm); 

  self.InicializarVistaModelo = function (event) {
    var target = `${self.IdForm} #RazonSocialProveedor_1`;
  ...
  self.ValidarAutoCompletadoProveedor = function (data, event) {
    if (event) {
      var $inputProveedor = self.form.find("#RazonSocialProveedor_1");
      var $evento = { target: `${self.IdForm} #RazonSocialProveedor_1` };
  ...
  self.DescargarReporteExcel = function (data, event) {
    if (event) {
      ...
      var url = SITE_URL + '/Reporte/CuentaPago/cReporteDetalladoCuentasPorPagar/GenerarReporteEXCEL';
  ...
  self.DescargarReportePdf = function (data, event) {
    if (event) {
      var datajs = self.ParseDataReporte(data, event);
      var url = SITE_URL + '/Reporte/CuentaPago/cReporteDetalladoCuentasPorPagar/GenerarReportePDF';
  ...
  self.MostrarReportePantalla = function (data, event) {
    if (event) {
      var datajs = self.ParseDataReporte(data, event);
      var url = SITE_URL + '/Reporte/CuentaPago/cReporteDetalladoCuentasPorPagar/GenerarReportePANTALLA';
...
var MappingReporteDetalladoCuentasPorPagar = {
  'Filtro': {
    create: function (options) {
      if (options)
        return new VistaModeloReporteDetalladoCuentasPorPagar(options.data);
    }
  }
}

IndexReporteDetalladoCuentasPorPagar = function (data) {
  var self = this;
  ko.mapping.fromJS(data, MappingReporteDetalladoCuentasPorPagar, self);

  self.Inicializar = function () {
    vistaModeloReporte.ReporteDetalladoCuentasPorPagar.Filtro.InicializarVistaModelo(window);
  }
}
