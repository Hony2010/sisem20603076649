VistaModeloReporteDocumentosPorPagar = function (data) {
  ...
  self.ValidarAutoCompletadoProveedor = function (data, event) {
    if (event) {
      var $inputProveedor = self.form.find("#RazonSocialProveedor_3");
      var $evento = { target: `${self.IdForm} #RazonSocialProveedor_3` };
  ...
  self.DescargarReporteExcel = function (data, event) {
    if (event) {
      ...
      var url = SITE_URL + '/Reporte/CuentaPago/cReporteDocumentosPorPagar/GenerarReporteEXCEL';
  ...
  self.DescargarReportePdf = function (data, event) {
    if (event) {
      var datajs = self.ParseDataReporte(data, event);
      var url = SITE_URL + '/Reporte/CuentaPago/cReporteDocumentosPorPagar/GenerarReportePDF';
  ...
  self.MostrarReportePantalla = function (data, event) {
    if (event) {
      var datajs = self.ParseDataReporte(data, event);
      var url = SITE_URL + '/Reporte/CuentaPago/cReporteDocumentosPorPagar/GenerarReportePANTALLA';
...
var MappingReporteDocumentosPorPagar = {
  'Filtro': {
    create: function (options) {
      if (options)
        return new VistaModeloReporteDocumentosPorPagar(options.data);
    }
  }
}

IndexReporteDocumentosPorPagar = function (data) {
  var self = this;
  ko.mapping.fromJS(data, MappingReporteDocumentosPorPagar, self);

  self.Inicializar = function () {
    vistaModeloReporte.ReporteDocumentosPorPagar.Filtro.InicializarVistaModelo(window);
  }
}
