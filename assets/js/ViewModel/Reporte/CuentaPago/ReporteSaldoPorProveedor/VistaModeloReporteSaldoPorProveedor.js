VistaModeloReporteSaldoPorProveedor = function (data) {
  var self = this;
  ko.mapping.fromJS(data, MappingReporteSaldoPorProveedor, self);

  self.RazonSocial = ko.observable('');
  self.NumeroDocumentoIdentidad = ko.observable('');
  self.IdForm = "#FormReporteSaldoPorProveedor";
  self.form = $(self.IdForm);

  self.InicializarVistaModelo = function (event) {
    var target = `${self.IdForm} #RazonSocialProveedor_5`;
  ...
  self.ValidarAutoCompletadoProveedor = function (data, event) {
    if (event) {
      var $inputProveedor = self.form.find("#RazonSocialProveedor_5");
      var $evento = { target: `${self.IdForm} #RazonSocialProveedor_5` };
  ...
  self.DescargarReporteExcel = function (data, event) {
    if (event) {
      ...
      var url = SITE_URL + '/Reporte/CuentaPago/cReporteSaldoPorProveedor/GenerarReporteEXCEL';
  ...
  self.DescargarReportePdf = function (data, event) {
    if (event) {
      var datajs = self.ParseDataReporte(data, event);
      var url = SITE_URL + '/Reporte/CuentaPago/cReporteSaldoPorProveedor/GenerarReportePDF';
  ...
  self.MostrarReportePantalla = function (data, event) {
    if (event) {
      var datajs = self.ParseDataReporte(data, event);
      var url = SITE_URL + '/Reporte/CuentaPago/cReporteSaldoPorProveedor/GenerarReportePANTALLA';
...
var MappingReporteSaldoPorProveedor = {
  'Filtro': {
    create: function (options) {
      if (options)
        return new VistaModeloReporteSaldoPorProveedor(options.data);
    }
  }
}

IndexReporteSaldoPorProveedor = function (data) {
  var self = this;
  ko.mapping.fromJS(data, MappingReporteSaldoPorProveedor, self);

  self.Inicializar = function () {
    vistaModeloReporte.ReporteSaldoPorProveedor.Filtro.InicializarVistaModelo(window);
  }
}
