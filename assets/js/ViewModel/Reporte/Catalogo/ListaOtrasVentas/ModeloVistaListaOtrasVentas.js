BuscadorModel_OtrasVentas = function (data) {
    var self = this;
    ko.mapping.fromJS(data, {}, self);

    self.OnClickBtnReportes = function (data,event) {
      var $data = {};
      $data.NombreArchivoJasper = self.NombreArchivoJasper_OtrasVentas();
      $data.NombreArchivoReporte = self.NombreArchivoReporte_OtrasVentas();
    ...
    self.DescargarReporte = function (datajs,event) {
      if (event)
      {
        ...
        $.ajax({
          ...
          data : datajs,
          dataType: "json",
          url: SITE_URL+'/Reporte/Catalogo/cListaOtrasVentas/'+nombre,
    ...
    self.MostrarReporte = function (datajs,event) {
      if (event)
      {
        ...
        $.ajax({
          ...
          data : datajs,
          dataType: "json",
          url: SITE_URL+'/Reporte/Catalogo/cListaOtrasVentas/GenerarReportePANTALLA',
          success: function (data) {
            $("#loader").hide();
            if(data != "")
            {
              document.getElementById("contenedorpdf").innerHTML='<iframe class="embed-responsive-item" src="'+SERVER_URL+'assets/reportes/'+self.NombreArchivoReporte_OtrasVentas()+'.pdf"></iframe>';
...
var MappingOtrasVentas = {
    'Buscador': {
        create: function (options) {
            if (options)
              return new BuscadorModel_OtrasVentas(options.data);
            }
    }
}
