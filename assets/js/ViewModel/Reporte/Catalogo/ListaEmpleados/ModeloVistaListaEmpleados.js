BuscadorModel_Empleados = function (data) {
    var self = this;
    ko.mapping.fromJS(data, {}, self);

    self.OnClickBtnReportes = function (data,event) {
      var $data = {};
      $data.NombreArchivoJasper = self.NombreArchivoJasper_Empleados();
      $data.NombreArchivoReporte = self.NombreArchivoReporte_Empleados();
    ...
    self.DescargarReporte = function (datajs,event) {
      if (event)
      {
        ...
        $.ajax({
          ...
          data : datajs,
          dataType: "json",
          url: SITE_URL+'/Reporte/Catalogo/cListaEmpleados/'+nombre,
    ...
    self.MostrarReporte = function (datajs,event) {
      if (event)
      {
        ...
        $.ajax({
          ...
          data : datajs,
          dataType: "json",
          url: SITE_URL+'/Reporte/Catalogo/cListaEmpleados/GenerarReportePANTALLA',
          success: function (data) {
            $("#loader").hide();
            if(data != "")
            {
              document.getElementById("contenedorpdf").innerHTML='<iframe class="embed-responsive-item" src="'+SERVER_URL+'assets/reportes/'+self.NombreArchivoReporte_Empleados()+'.pdf"></iframe>';
...
var MappingEmpleados = {
    'Buscador': {
        create: function (options) {
            if (options)
              return new BuscadorModel_Empleados(options.data);
            }
    }
}
