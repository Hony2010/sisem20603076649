BuscadorModel_ActivosFijos = function (data) {
    var self = this;
    ko.mapping.fromJS(data, {}, self);

    self.OnClickBtnReportes = function (data,event) {
      var $data = {};
      $data.NombreArchivoJasper = self.NombreArchivoJasper_ActivosFijos();
      $data.NombreArchivoReporte = self.NombreArchivoReporte_ActivosFijos();
    ...
    self.DescargarReporte = function (datajs,event) {
      if (event)
      {
        ...
        $.ajax({
          ...
          data : datajs,
          dataType: "json",
          url: SITE_URL+'/Reporte/Catalogo/cListaActivosFijos/'+nombre,
    ...
    self.MostrarReporte = function (datajs,event) {
      if (event)
      {
        ...
        $.ajax({
          ...
          data : datajs,
          dataType: "json",
          url: SITE_URL+'/Reporte/Catalogo/cListaActivosFijos/GenerarReportePANTALLA',
          success: function (data) {
            $("#loader").hide();
            if(data != "")
            {
              document.getElementById("contenedorpdf").innerHTML='<iframe class="embed-responsive-item" src="'+SERVER_URL+'assets/reportes/'+self.NombreArchivoReporte_ActivosFijos()+'.pdf"></iframe>';
...
var MappingActivosFijos = {
    'Buscador': {
        create: function (options) {
            if (options)
              return new BuscadorModel_ActivosFijos(options.data);
            }
    }
}
