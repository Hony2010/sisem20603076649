BuscadorModel_CostosAgregados = function (data) {
    var self = this;
    ko.mapping.fromJS(data, {}, self);

    self.OnClickBtnReportes = function (data,event) {
      var $data = {};
      $data.NombreArchivoJasper = self.NombreArchivoJasper_CostosAgregados();
      $data.NombreArchivoReporte = self.NombreArchivoReporte_CostosAgregados();
    ...
    self.DescargarReporte = function (datajs,event) {
      if (event)
      {
        ...
        $.ajax({
          ...
          data : datajs,
          dataType: "json",
          url: SITE_URL+'/Reporte/Catalogo/cListaCostosAgregados/'+nombre,
    ...
    self.MostrarReporte = function (datajs,event) {
      if (event)
      {
        ...
        $.ajax({
          ...
          data : datajs,
          dataType: "json",
          url: SITE_URL+'/Reporte/Catalogo/cListaCostosAgregados/GenerarReportePANTALLA',
          success: function (data) {
            $("#loader").hide();
            if(data != "")
            {
              document.getElementById("contenedorpdf").innerHTML='<iframe class="embed-responsive-item" src="'+SERVER_URL+'assets/reportes/'+self.NombreArchivoReporte_CostosAgregados()+'.pdf"></iframe>';
...
var MappingCostosAgregados = {
    'Buscador': {
        create: function (options) {
            if (options)
              return new BuscadorModel_CostosAgregados(options.data);
            }
    }
}
