VistaModeloReporteModeloMovimientoCuentasPorCobrar = function (data) {
  var self = this;
  ko.mapping.fromJS(data, {}, self);

  self.RazonSocial = ko.observable('');
  self.NumeroDocumentoIdentidad = ko.observable('');
  self.IdForm = "#FormReporteModeloMovimientoCuentasPorCobrar";
  self.form = $(self.IdForm);

  self.InicializarVistaModelo = function (event) {
    var target = `${self.IdForm} #RazonSocialCliente_4`;
  ...
  self.ValidarAutoCompletadoCliente = function (data, event) {
    if (event) {
      var $inputCliente = self.form.find("#RazonSocialCliente_4");
      var $evento = { target: `${self.IdForm} #RazonSocialCliente_4` };
  ...
  self.DescargarReporteExcel = function (data, event) {
    if (event) {
      ...
      var url = SITE_URL + '/Reporte/CuentaCobranza/cReporteModeloMovimientoCuentasPorCobrar/GenerarReporteEXCEL';
  ...
  self.DescargarReportePdf = function (data, event) {
    if (event) {
      var datajs = self.ParseDataReporte(data, event);
      var url = SITE_URL + '/Reporte/CuentaCobranza/cReporteModeloMovimientoCuentasPorCobrar/GenerarReportePDF';
  ...
  self.MostrarReportePantalla = function (data, event) {
    if (event) {
      var datajs = self.ParseDataReporte(data, event);
      var url = SITE_URL + '/Reporte/CuentaCobranza/cReporteModeloMovimientoCuentasPorCobrar/GenerarReportePANTALLA';
  ...
  self.SeleccionarTodosVendedores = function (data, event) {
    if (event) {
      ...
      usuarios.forEach(function (value, key) {
        $('#' + value.IdUsuario + '_Vendedor_mcpc').prop('checked', selectorTodos);
  ...
  self.SeleccionarVendedor = function (data, event) {
    if (event) {
      ...
      if (self.NumeroVendedoresSeleccionados() == self.TotalVendedores()) {
        $('#SelectorVendedores_mcpc').prop('checked', true);
      } else {
        $('#SelectorVendedores_mcpc').prop('checked', false);
      }
...
var MappingReporteModeloMovimientoCuentasPorCobrar = {
  'Filtro': {
    create: function (options) {
      if (options)
        return new VistaModeloReporteModeloMovimientoCuentasPorCobrar(options.data);
    }
  }
}

IndexReporteModeloMovimientoCuentasPorCobrar = function (data) {
  var self = this;
  ko.mapping.fromJS(data, MappingReporteModeloMovimientoCuentasPorCobrar, self);

  self.Inicializar = function () {
    vistaModeloReporte.ReporteModeloMovimientoCuentasPorCobrar.Filtro.InicializarVistaModelo(window);
  }
}
