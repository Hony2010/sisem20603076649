ModeloOtroDocumentoEgreso = function (data) {
  ...
  self.NuevoOtroDocumentoEgreso = function(data,event) {
    if(event) {
      ...
      self.OtroDocumentoEgresoInicial = ko.mapping.toJS(data,mappingfinal);
      self.opcionProceso(opcionProceso.Nuevo);
    }
  }

  self.EditarOtroDocumentoEgreso = function(data,event) {
    if(event) {
      self.EstaProcesado(false);
      var copia = Knockout.CopiarObjeto(data);
      ko.mapping.fromJS(copia,{},self);
  ...
  self.GuardarOtroDocumentoEgreso = function (event,callback) {
      if(event) {
        var _mappingIgnore  =ko.toJS(mappingIgnore);
        var mappingfinal = Object.assign(_mappingIgnore);
  ...
  self.Insertar = function(data,event,callback) {
    if (event) {
      $.ajax({
        ...
        data : data,
        dataType: "json",
        url: SITE_URL+'/Caja/cDocumentoEgreso/InsertarDocumentoEgreso',
  ...
  self.Actualizar = function(data,event,callback) {
    if (event) {
      $.ajax({
        ...
        data : data,
        dataType: "json",
        url: SITE_URL+'/Caja/cDocumentoEgreso/ActualizarDocumentoEgreso',
        success: function (data) {
          self.EstaProcesado(true);
          callback(data,event);
        },
  ...
  self.EliminarDocumentoEgreso = function(data,event,callback) {
    if (event) {
      ...
      $.ajax({
              ...
              data : datajs,
              dataType: "json",
              url: SITE_URL+'/Caja/cDocumentoEgreso/BorrarDocumentoEgreso',
              success: function (data) {
                callback(data,event);
            },
            error : function (jqXHR, textStatus, errorThrown) {
  ...
  self.AnularDocumentoEgreso = function(data,event,callback) {
    if (event) {
      var objeto = ko.mapping.toJS(Knockout.CopiarObjeto(data), mappingIgnore);
      var datajs = {Data: JSON.stringify(objeto)};
      self.opcionProceso(opcionProceso.Eliminacion);
      $.ajax({
              ...
              data : datajs,
              dataType: "json",
              url: SITE_URL+'/Caja/cDocumentoEgreso/AnularDocumentoEgreso',
              success: function (data) {
                callback(data,event);
            },
            error : function (jqXHR, textStatus, errorThrown) {
