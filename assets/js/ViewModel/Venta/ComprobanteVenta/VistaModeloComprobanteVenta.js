VistaModeloComprobanteVenta = function (data, options) {
	...
	if (self.hasOwnProperty("ParametroCamposConEnvioYGestion")) {
		...
		self.NombreCampoSubTotal = ko.observable(
			self.ParametroCamposConEnvioYGestion() == "1" ? "Sumatoria" : "SubTotal"
		);
	} else {
		self.NombreCampoSubTotal = ko.observable("");
	}
	...
	self.DivFooterVenta = ko.pureComputed(function () {
		if (self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_ORDEN_PEDIDO) {
			if (
				self.ParametroCamposConEnvioYGestion() == "1" &&
				self.ParametroCampoACuenta() == "1"
			) {
				return "col-md-2";
			} else if (
				self.ParametroCamposConEnvioYGestion() == "1" &&
				self.ParametroCampoACuenta() == "0"
			) {
				return "col-md-4";
			} else if (
				self.ParametroCamposConEnvioYGestion() == "0" &&
				self.ParametroCampoACuenta() == "1"
			) {
				return "col-md-4";
			} else {
	...
	self.InicializarVistaModelo = function (data, event, editar = true) {
		if (event) {
			...
			if (self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_FACTURA) {
				$form
					...
					.autoCompletadoCliente(
						event,
						self.ValidarAutoCompletadoCliente,
						CODIGO_TIPO_DOCUMENTO_IDENTIDAD.RUC,
						target
					);
			} else if (
				self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_BOLETA &&
				self.ParametroFiltroClienteSinRuc() == 1
			) {
				$form
					.find("#Cliente")
					.autoCompletadoCliente(
				...
				$form
					...
					.autoCompletadoCliente(
						event,
						self.ValidarAutoCompletadoCliente,
						CODIGO_TIPO_DOCUMENTO_IDENTIDAD.TODOS,
						target
					);
			}
			...
			$form
				.find("#RazonSocialDestinatario")
				.autoCompletadoCliente(
					event,
					self.ValidarAutoCompletadoRazonSocialDestinatario,
					CODIGO_TIPO_DOCUMENTO_IDENTIDAD.DNI,
					target2
				);
			...
			if (self.ParametroRubroLubricante() == 1) {
				...
				$form
					.find("#NumeroPlaca")
					.autoCompletadoPlacaVehiculo(
						dataNumeroPlaca,
						event,
						self.ValidarAutoCompletadoNumeroPlacaVehiculo
					);
			}
			...
			$form
				.find("#FechaEmision")
				.inputmask({ mask: "99/99/9999", positionCaretOnTab: false });
			$form
				.find("#FechaVencimiento")
				.inputmask({ mask: "99/99/9999", positionCaretOnTab: false });
			$form
				.find("#FechaMovimientoAlmacen")
				.inputmask({ mask: "99/99/9999", positionCaretOnTab: false });
			$form
				.find(".fecha")
				.inputmask({ mask: "99/99/9999", positionCaretOnTab: false });
			...
			if (self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_FACTURA) {
				self.Sedes.remove(function (item) {
					return item.IndicadorAlmacenZofra() == 1;
				});
			} else if (self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_BOLETA) {
				if (
					self.IdSubTipoDocumento() == ID_SUBTIPO_DOCUMENTO_BOLETA_T ||
					self.IdSubTipoDocumento() == ID_SUBTIPO_DOCUMENTO_BOLETA_Z
				) {
					self.Sedes.remove(function (item) {
						return item.IndicadorAlmacenZofra() == 0;
					});
				} else {
					self.Sedes.remove(function (item) {
						return item.IndicadorAlmacenZofra() == 1;
			...
			if (editar) {
				if (
					self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_BOLETA ||
					self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_ORDEN_PEDIDO
				) {
					if (
						self.ParametroVistaVenta() == 1 ||
						!(self.TipoVenta() == TIPO_VENTA.SERVICIOS)
					) {
			...
			if (self.ParametroRubroLubricante() == 1) {
				...
				$form
					.find("#NombreRadioTaxi")
					.autoCompletadoRadioTaxis(
						inputRadioTaxi,
						event,
						self.ValidarAutoCompletadoRadioTaxi
					);
			}
			...
			$form
				.find("#DocumentoVentaProforma")
				.autoCompletadoComprobanteVentaProforma(
					IdTarget,
					event,
					self.ValidarAutoCompletadoComprobanteVentaProforma
				);
			...
			self.OnChangeCheckEstadoRetencionIGV(data,event);

			self.ObtenerTipoCambioSunat(self, event);
			ko.utils.arrayForEach(self.CuotasPagoClienteComprobanteVenta(), function(item) {
				item.InicializarVistaModelo(item,event);
			});
		...
		if (event) {
			$.formUtils.addValidator({
				...
				validatorFunction: function (value, $el) {
					...
					var resultado =
						value.toUpperCase() === texto.toUpperCase() &&
						value.toUpperCase() !== ""
							? true
							: false;
					return resultado;
			...
			$.formUtils.addValidator({
				...
				validatorFunction: function (value, $el) {
					var texto = $el.attr("data-validation-text-found");
					var resultado =
						value.toUpperCase() === texto.toUpperCase() ? true : false;
					return resultado;
				},
			...
			$.formUtils.addValidator({
				...
				validatorFunction: function (value, $el, config, language, $form) {
					if (value !== "") {
						var dateFormat =
							$el.valAttr("format") || config.dateFormat || "yyyy-mm-dd";
						var addMissingLeadingZeros =
						...
						if (self.IdFormaPago() === ID_FORMA_PAGO_CREDITO) return false;
						else return true;
					}
				},
			});
			...
			$.formUtils.addValidator({
				...
				validatorFunction: function (value, $el, config, language, $form) {
					...
					var texto = $referencia
						.attr("data-validation-text-found")
						.toUpperCase();
					var resultado =
						value.toUpperCase() === texto && value.toUpperCase() !== ""
							? true
							: false;
					return resultado;
				},
	...
	self.OnClickTipoVenta = function (data, event) {
		if (event) {
			if (self.CambiosFormulario() == true) {
				alertify.confirm(
					self.titulo,
					"Se perderá el registro actual, ¿Desea cambiar de todas formas?",
					function () {
	...
	self.OnChangeMoneda = function (data, event) {
		if (event) {
			var texto = $(self.Options.IDPanelHeader)
				.find("#combo-moneda option:selected")
				.text();
			...
			if (self.IdMoneda() == ID_MONEDA_DOLARES && valorTipoCambio <= 0) {
				...
				self.TipoCambio.ObtenerTipoCambio(data, function ($data) {
					...
					if ($data) {
						...
						alertify.alert(
							self.titulo,
							"No se encontro un tipo de cambio para la fecha emision"
						);
					}
				});
	...
	self.OnRefrescar = function (data, event, esporeliminacion) {
		if (event) {
			if (!$form.hasClass("selector-blocked")) {
	...
	self.OnClickBtnNuevoCliente = function (data, event, dataCliente) {
		if (event) {
			dataCliente.OnNuevo(
				dataCliente.ClienteNuevo,
				event,
				self.PostCerrarCliente
			);
	...
	self.OnClickBtnNuevaMercaderia = function (data, event, dataMercaderia) {
		if (event) {
			dataMercaderia.OnNuevaMercaderia(
				dataMercaderia.MercaderiaNueva,
				event,
				self.PostCerrarMercaderia
			);
			return true;
		}
	};
	...
	self.OnClickBtnBuscadorMercaderia = function (data, event, dataBase) {
		if (event) {
			...
			dataBase.BusquedaAvanzadaProducto.InicializarVistaModelo(
				data,
				event,
				self
			);
	...
	self.OnClickBtnBuscadorMercaderiaListaSimple = function (
		data,
		event,
		dataBase
	) {
		if (event) {
			...
			$("#BuscadorMercaderiaListaSimple #spanNombreSede").text(
				self.NombreSedeAlmacen()
			);
			//dataBase.Mercaderias([]);
			dataBase.BusquedaAvanzadaProducto.InicializarVistaModelo(
				data,
				event,
				self,
				self.OnClickAgregarMercaderiaImagen
			);
	...
	self.OnClickAgregarMercaderiaImagen = function (data, event) {
		if (event) {
			...
			if (
				self.IndicadorPermisoStockNegativo() == 0 &&
				parseFloatAvanzado(objeto.StockProducto) <= 0
			) {
				alertify.alert(
					"Comprobante Venta",
					"Este producto no cuenta con Stock para proceder con la Venta..",
					function () {}
				);
				return false;
			}

			var detalleFiltrado = ko.utils.arrayFilter(
			...
			self.DetallesComprobanteVenta.remove(function (item) {
				return item.IdProducto() == null;
			});

			objeto.CostoUnitario =
				objeto.CostoUnitario == "" || objeto.CostoUnitario == null
					? "0"
					: objeto.CostoUnitario;
			objeto.DescuentoItem =
				objeto.DescuentoItem == "" || objeto.DescuentoItem == null
					? "0"
					: objeto.DescuentoItem;
			objeto.DescuentoUnitario =
				objeto.DescuentoUnitario == "" || objeto.DescuentoUnitario == null
					? "0"
					: objeto.DescuentoUnitario;

			if (detalleFiltrado.length > 0) {
				var producto = detalleFiltrado[0];
				var cantidadaDetalleFiltrado =
					parseFloatAvanzado(producto.Cantidad()) +
					parseFloatAvanzado(objeto.Cantidad);
				var precioUnitarioDetalleFiltrado = parseFloatAvanzado(
					producto.PrecioUnitario()
				);
				producto.Cantidad(parseFloatAvanzado(cantidadaDetalleFiltrado));
				producto.SubTotal(
					parseFloatAvanzado(
						cantidadaDetalleFiltrado * precioUnitarioDetalleFiltrado
					)
				);
			} else {
				...
				$objeto.IdDetalleComprobanteVenta =
					producto.IdDetalleComprobanteVenta();
				var dataItem = self.ObtenerProductoPorIdProducto(producto, event);

				if (dataItem) {
					if (
						self.TipoVenta() == TIPO_VENTA.MERCADERIAS &&
						self.ParametroLote() == 1
					) {
			...
			$form
				.find(producto.InputCodigoMercaderia())
				.attr("data-validation-found", "true");
			$form
				.find(producto.InputCodigoMercaderia())
				.attr(
					"data-validation-text-found",
					$form.find(producto.InputProducto()).val()
				);
			$form
				.find(producto.InputNumeroLote())
				.attr("data-validation-found", "false");
			$form
				.find(producto.InputNumeroDocumentoSalidaZofra())
				.attr("data-validation-found", "false");
			$form
				.find(producto.InputNumeroDua())
				.attr("data-validation-found", "false");
	...
	self.OnClickAgregarMercaderiaImagenPuntoVenta = function (data, event) {
		if (event) {
			...
			if (
				self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_BOLETA &&
				parseFloatAvanzado(self.Total()) >= MONTO_MAXIMO_BOLETA
			) {
				...
				alertify.alert(
					self.titulo,
					"El monto de la Boleta es >= S/ 700.00, se necesita colocar los datos del Cliente.",
					function () {
			...
			if (detalleFiltrado.length > 0) {
				var cantidadaDetalleFiltrado =
						parseFloatAvanzado(detalleFiltrado[0].Cantidad()) + 1,
					precioUnitarioDetalleFiltrado = parseFloatAvanzado(
						detalleFiltrado[0].PrecioUnitario()
					);
				detalleFiltrado[0].Cantidad(
					parseFloatAvanzado(cantidadaDetalleFiltrado)
				);
				detalleFiltrado[0].SubTotal(
					parseFloatAvanzado(
						cantidadaDetalleFiltrado * precioUnitarioDetalleFiltrado
					)
				);
				...
				var id =
					"#" + detalleFiltrado[0].IdDetalleComprobanteVenta() + "_tr_detalle";
			} else {
				var detalle = ko.mapping.toJS(self.NuevoDetalleComprobanteVenta);
				objeto = Object.assign(detalle, objeto);
				objeto.Cantidad = "1";
	...
	self.OnClickAgregarMercaderiaImagenComanda = function (data, event) {
		if (event) {
			...
			var idanotacionplato = data.IdAnotacionPlato
				? data.IdAnotacionPlato()
				: "";
			...
			var detalleFiltrado = ko.utils.arrayFilter(
				...
				function (item) {
					if (item.IdProducto() == objeto.IdProducto) {
						if (
							item.IndicadorImpresion() != ESTADO_INDICADOR_IMPRESION.ENVIADO
						) {
			...
			if (detalleFiltrado.length > 0) {
				...
				objeto.Observacion = data.NombreAnotacionPlato
					? data.NombreAnotacionPlato()
					: "";
	...
	self.AgregarProductoBonificado = function (data, event) {
		if (event) {
			var detalleFiltrado = ko.utils.arrayFilter(
				...
				function (item) {
					return (
						...
						item.IndicadorOperacionGratuita() == 1
					);
				}
			);
			...
			if (detalleFiltrado.length > 0) {
				...
				var objeto = Object.assign(
					ko.mapping.toJS(self.NuevoDetalleComprobanteVenta),
					data
				);
				objeto.Cantidad = data.Cantidad;
				var valorreferencial =
					parseFloatAvanzado(data.PrecioUnitario) /
					(1 + parseFloatAvanzado(self.TasaIGV()));
				var valorreferencialitem =
					parseFloatAvanzado(objeto.Cantidad) * valorreferencial;
				var igvreferencialitem =
					valorreferencialitem * parseFloatAvanzado(self.TasaIGV());
				...
				detalle.NombreLargoProducto(
					detalle.NombreLargoProducto() + " (BONIFICACION)"
				);
	...
	self.PostCerrarCliente = function (dataCliente, data, event) {
		if (event) {
			...
			if (self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_FACTURA) {
				if (
					cliente.CodigoDocumentoIdentidad ==
					CODIGO_TIPO_DOCUMENTO_IDENTIDAD.RUC
				) {
				...
				if (
					self.ParametroFiltroClienteSinRuc() == 1 &&
					cliente.NumeroDocumentoIdentidad.length == MAXIMO_DIGITOS_RUC
				) {
					$("#combo-formapago").focus();
				} else {
					$form.find("#CheckCliente").prop("checked", true);
	...
	self.OnClickBtnBusquedaProducto = function (data, event, dataCliente) {
		if (event) {
			dataCliente.OnNuevo(
				dataCliente.ClienteNuevo,
				event,
				self.PostCerrarBusquedaProducto
			);
			dataCliente.IdTipoDocumentoIdentidad(ID_TIPO_DOCUMENTO_IDENTIDAD_RUC);
			dataCliente.Show(event);
			return true;
	...
	self.Nuevo = function (data, event, callback) {
		if (event) {
			...
			$form.find("#CheckEstadoDetraccion").prop("checked", false);
			self.OnChangeCheckEstadoDetraccion(data,event);
			setTimeout(function () {
				if (
					self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_BOLETA ||
					self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_ORDEN_PEDIDO
	...
	self.Editar = function (data, event, callback, blocked) {
		if (event) {
			if (self.IndicadorReseteoFormulario === true)
				$form.resetearValidaciones(); //'#formComprobanteVenta'
			if (callback) self.callback = callback;
			...
			if (self.NumeroDocumentoIdentidad() == "") {
				$form
					.find("#Cliente")
					.attr("data-validation-text-found", self.RazonSocial());
			} else {
				$form
					.find("#Cliente")
					.attr(
						"data-validation-text-found",
						self.NumeroDocumentoIdentidad() + " - " + self.RazonSocial()
					);
			}
			$form
				.find("#RazonSocialDestinatario")
				.attr("data-validation-text-found", self.RazonSocialDestinatario());

			$form
				.find("#DocumentoVentaReferencia")
				.attr("data-validation-text-found", self.DocumentoVentaReferencia());
			$form
				.find("#DocumentoVentaProforma")
				.attr("data-validation-text-found", self.DocumentoVentaProforma());
			$form
				.find("#NumeroPlaca")
				.attr("data-validation-text-found", self.NumeroPlaca());
			$form
				.find("#NombreRadioTaxi")
				.attr("data-validation-text-found", self.NombreRadioTaxi());
			...
			setTimeout(function () {
				if (self.NumeroDocumentoIdentidad() == "") {
					...
					$form
						.find("#Cliente")
						.val(self.NumeroDocumentoIdentidad() + " - " + self.RazonSocial());
				}
				...
				if (self.IdTipoVenta() == TIPO_VENTA.SERVICIOS) {
					self.CheckDestinatario(
						self.IdDestinatario() == null || self.IdDestinatario() == "" ? 1 : 0
					);
			...
			$form.find("#CheckEstadoDetraccion").prop("checked",  self.EstadoDetraccion());

			$("#loader").show();
			self.ConsultarDetallesComprobanteVenta(
				data,
				event,
				function ($data, $event) {
					self.InicializarVistaModeloDetalle(undefined, event);
						...
						ko.utils.arrayForEach(
							...
							function (item) {
								if (item.IdProducto() != null)
									self.CopiaIdProductosDetalle.push(item.IdProducto());
								...
								if (dataItem) {
									...
									if (
										self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_BOLETA &&
										(self.IdSubTipoDocumento() ==
											ID_SUBTIPO_DOCUMENTO_BOLETA_T ||
											self.IdSubTipoDocumento() ==
												ID_SUBTIPO_DOCUMENTO_BOLETA_Z)
									) {
								...
								$form
									.find(item.InputCodigoMercaderia())
									.attr("data-validation-found", "true");
								$form
									.find(item.InputCodigoMercaderia())
									.attr(
										"data-validation-text-found",
										$form.find(item.InputProducto()).val()
									);
								$form
									.find(item.InputNumeroLote())
									.attr("data-validation-found", "true");
								$form
									.find(item.InputNumeroDocumentoSalidaZofra())
									.attr("data-validation-found", "true");
								$form
									.find(item.InputNumeroDua())
									.attr("data-validation-found", "true");

								if (item.IndicadorOperacionGratuita() == 1) {
									$form
										.find(item.InputSubTotal())
					...
					self.IndicadorCrearProducto(
						parseFloat(self.IndicadorCrearProducto())
					);
	...
	self.AbrirPreVenta = function (data, event, callback) {
		if (event) {
			...
			ko.utils.arrayForEach(data.DetallesComprobanteVenta, function (item) {
				item.Cantidad =
					parseFloatAvanzado(item.SaldoPendientePreVenta) <
					parseFloatAvanzado(item.Cantidad)
						? item.SaldoPendientePreVenta
						: item.Cantidad;
				item.PrecioUnitario = parseFloatAvanzado(item.PrecioUnitario).toFixed(
					CANTIDAD_DECIMALES_VENTA.PRECIO_UNITARIO
				);
				var objeto = self.DetallesComprobanteVenta.Agregar(
					new VistaModeloDetalleComprobanteVenta(item, self),
					event
				);
	...
	self.Guardar = function (data, event) {
		if (event) {
			...
			if (self.ParametroSauna() == 1) {
				if (
					(self.IdGenero() == "" || self.IdCasillero() == "") &&
					self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_ORDEN_PEDIDO
				) {
					alertify.alert(
						self.titulo,
			...
			if ($form.isValid() === false) {
				...
				alertify.alert(
					"Error en Validación",
					"Existe aun datos inválidos , por favor de corregirlo.",
					function () {
						setTimeout(function () {
				...
				var filtrado = ko.utils.arrayFilter(
					self.DetallesComprobanteVenta(),
					function (item) {
						return item.IdProducto() != null;
					}
				...
				if (self.OpcionComprobantesAutomaticos() == false) {
					...
					var cantidadComprobantesAutomaticos = parseFloatAvanzado(
							self.CantidadComprobantesAutomaticos()
						),
						parametroMaxComprobantesAutomaticos = parseFloatAvanzado(
							self.ParametroMaxComprobantesAutomaticos()
						);

					if (
						cantidadComprobantesAutomaticos >
						parametroMaxComprobantesAutomaticos
					) {
						alertify.alert(
							"Validacion!",
							"La cantidad maxima de comprobantes automaticos es " +
								self.ParametroMaxComprobantesAutomaticos(),
							function () {
								setTimeout(() => {
					...
					if (cantidadComprobantesAutomaticos <= 0) {
						alertify.alert(
							"Validacion!",
							"La cantidad minima de comprobantes automaticos es 2",
							function () {
								setTimeout(() => {
									$("#CantidadComprobantesAutomaticos").focus();
								}, 350);
					...
					for (var i = 0; i < self.CantidadComprobantesAutomaticos(); i++) {
						self.GuardarComprobanteVenta(
							event,
							self.PostGuardarComprobantesAutomaticos
						);
					}
				}
			}
	...
	self.ValidarGuardadoPreVenta = function (data, event) {
		if (event) {
			...
			if ($form.isValid() === false) {
				...
				if (
					self.DetallesComprobanteVenta().length <= 0 &&
					self.IdTipoDocumento() != ID_TIPO_DOCUMENTO_COMANDA
				) {
					alertify.alert(
						"Validación",
						"Debe ingresar por lo menos 1 ítem.",
						function () {}
					);
					return false;
				}
	...
	self.PostGuardarComanda = function (data, event) {
		if (event) {
			if (data.error) {
				...
				var itemsImprimido = ko.utils.arrayFilter(
					self.DetallesComprobanteVenta(),
					function (item) {
						return (
							item.IndicadorImpresion() == ESTADO_INDICADOR_IMPRESION.ENVIADO
				...
				alertify.confirm(
					self.titulo,
					"Se guardó el comprobante " +
						self.SerieDocumento() +
						" - " +
						self.NumeroDocumento() +
						" correctamente. <br /> <b> ¿Desea enviar la comanda a cocina?</b><br />",
					function () {
						$("#loader").show();
		...
		if (event) {
			if (data.error) {
				...
				self.mensaje =
					...
					" - " +
					self.NumeroDocumento() +
					" correctamente. <br /> ¿Desea Imprimir la Pre Cuenta?";
				alertify.confirm(
					self.titulo,
					self.mensaje,
					function () {
		...
		if (event) {
			if (data.error) {
				...
				self.mensaje =
					...
					" - " +
					self.NumeroDocumento() +
					" correctamente. <br /> ¿Desea imprimir el comprobante?";
	...
	self.PostGuardarComprobantesAutomaticos = function (data, event) {
		if (event) {
			if (data.error) {
				...
				if (
					self.ComprobantesAutomaticos().length ==
					self.CantidadComprobantesAutomaticos()
				) {
					$("#loader").hide();
					alertify.confirm(
						...
						"¿Desea Imprimir los " +
							self.ComprobantesAutomaticos().length +
							" Comprobantes Automaticos?",
						function () {
							$("#loader").show();
							ko.utils.arrayFirst(
								self.ComprobantesAutomaticos(),
								function (item) {
	...
	self.PostGuardar = function (data, event) {
		if (event) {
			...
			if (data.error) {
				...
				if (
					self.IndicadorEnvioAutomaticoSUNAT() == 1 &&
					self.opcionProceso() == opcionProceso.Nuevo
				) {
					alertify.confirm(
						self.titulo,
						`Se registró el comprobante ${data.SerieDocumento}-${data.NumeroDocumento} correctamente ¿Desea Enviar a SUNAT e imprimir el documento?`,
						function () {
							$("#loader").show();
	...
	self.AnularComprobantePreVenta = function (data, event, callback) {
		if (event) {
			if (callback != undefined) self.callback = callback;
			self.AnularVentaDesdePreVenta(
				data,
				event,
				self.PostAnularComprobantePreVenta
			);
		}
	};
	...
	self.EliminarComprobantePreVenta = function (data, event, callback) {
		if (event) {
			if (callback != undefined) self.callback = callback;
			self.EliminarVentaDesdePreVenta(
				data,
				event,
				self.PostEliminarComprobantePreVenta
			);
		}
	};
	...
	self.PostAnularComprobantePreVenta = function (data, event) {
		if (event) {
			...
			if (!data.error) {
				alertify.alert(
					"Anulación de Comprobante de Venta",
	...
	self.PostEliminarComprobantePreVenta = function (data, event) {
		if (event) {
			...
			if (resultado.error === undefined) {
				...
				if (self.callback != undefined) self.callback(resultado, event);
			} else {
				alertify.alert(
	...
	self.EnviarEmailCliente = function (data, event) {
		if (event) {
			...
			if (
				(data_objeto.SerieDocumento.search(CODIGO_SERIE_BOLETA) >= 0 &&
					data_objeto.IdTipoDocumento == ID_TIPO_DOCUMENTO_BOLETA) ||
				(data_objeto.SerieDocumento.search(CODIGO_SERIE_FACTURA) >= 0 &&
					data_objeto.IdTipoDocumento == ID_TIPO_DOCUMENTO_FACTURA)
			) {
	...
	self.CalculoTotalVentaGravado = ko.computed(function () {
		var resultado = accounting.formatNumber(
			self.ValorVentaGravado(),
			NUMERO_DECIMALES_VENTA
		);
		return resultado;
	}, this);
	...
	self.DenominacionTotal = ko.computed(function () {
		...
		if (resultado != null) {
			...
			else self.MontoLetra(Number(resultado).DenominacionMonedaDOLARES());
		} else {
			self.MontoLetra("");
		}
		...
		if (self.IndicadorEstado() == ESTADO.ACTIVO) {
			if (self.IndicadorEstadoCPE() == ESTADO_CPE.GENERADO) {
				if (self.IndicadorEstadoResumenDiario() == ESTADO_CPE.ACEPTADO) {
					...
					if (
						self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_BOLETA &&
						self.CodigoEstado() != CODIGO_ESTADO.ANULADO
					) {
						return true;
					}
					return false;
    ...
    */
		...
		if (self.IndicadorEstado() == ESTADO.ACTIVO) {
			if (self.IndicadorEstadoCPE() == ESTADO_CPE.GENERADO) {
				if (self.IndicadorEstadoResumenDiario() == ESTADO_CPE.ACEPTADO) {
					...
					if (
						self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_BOLETA &&
						self.CodigoEstado() == CODIGO_ESTADO.MODIFICADO
					) {
						return false;
					}
					return true;
			...
			if ($form.find("#CheckCliente").length > 0) {
				if (
					...
					if ($form.find("#CheckCliente").prop("checked")) {
						$form
							.find("#Cliente")
							.attr("data-validation", "autocompletado_cliente");
	...
	self.CambiarClientesVariosPreCuenta = function (data, event) {
		if (event) {
			...
			$form
				.find("#ClientePreCuenta")
				.attr("data-validation-text-found", TEXTO_CLIENTES_VARIOS);
		}
	};
	...
	self.RefrescarBotonesDetalleComprobanteVenta = function (data, event) {
		if (event) {
			...
			if (indice === tamaño - 1) {
				...
				var InputOpcionMercaderia = self
					.DetallesComprobanteVenta()
					[indice].InputOpcionMercaderia();
				$(InputOpcion).show();
	...
	self.RemoverExcepcionValidaciones = function (data, event) {
		if (event) {
			...
			$form
				.find(ultimoItem.InputCodigoMercaderia())
				.attr("data-validation-optional", resultado);
			$form
				.find(ultimoItem.InputProducto())
				.attr("data-validation-optional", resultado);
			$form
				.find(ultimoItem.InputCantidad())
				.attr("data-validation-optional", resultado);
			$form
				.find(ultimoItem.InputNumeroLote())
				.attr("data-validation-optional", resultado);
			$form
				.find(ultimoItem.InputNumeroDua())
				.attr("data-validation-optional", resultado);
			$form
				.find(ultimoItem.InputPrecioUnitario())
				.attr("data-validation-optional", resultado);
			$form
				.find(ultimoItem.InputDescuentoUnitario())
				.attr("data-validation-optional", resultado);
			$form
				.find(ultimoItem.InputSubTotal())
				.attr("data-validation-optional", resultado);
			$form
				.find(ultimoItem.InputNumeroDocumentoSalidaZofra())
				.attr("data-validation-optional", resultado);
			$form
				.find(ultimoItem.InputObservacion())
				.attr("data-validation-optional", resultado);
		}
	};
	...
	self.OnQuitarFila = function (data, event) {
		if (event) {
			...
			var trfilas = $("#tablaDetalleComprobanteVenta")
				.find("tr")
				.find("button:visible");
			if (trfilas.length == 0) {
				setTimeout(function () {
	...
	self.OnQuitarProductoBonificado = function (data, event) {
		if (event) {
			var bonificaciones =
				data.ListaBonificaciones == undefined
					? []
					: ko.mapping.toJS(data.ListaBonificaciones());

			if (
				self.ParametroBonificacion() == 1 &&
				bonificaciones.length > 0 &&
				data.IndicadorOperacionGratuita() == 0
			) {
				bonificaciones.forEach(function (item) {
					var producto = ko.utils.arrayFirst(
						self.DetallesComprobanteVenta(),
						function (producto) {
	...
	self.OnClickBtnEliminarItemComanda = function (data, event) {
		if (event) {
			...
			if (self.IdRolUsuario() != ID_ROL_CAJERO) {
				alertify.alert(
					"ELIMINACIÓN DE ITEM DE COMANDA",
					"Este ítem ya fue enviado a cocina, Para anularlo deberá comunicarse con el Cajero",
					function functionName() {}
				);
				return false;
			}
				...
				function () {
					...
					self.EliminarItemComanda(data, event, function ($data, event) {
						...
						if (!$data.error) {
							alertify.alert(
								self.titulo,
								'Se Eliminó correctamente el producto <b>"' +
									data.NombreProducto() +
									'"</b>.',
								function () {
									self.OnQuitarFila(data, event);
	...
	self.OnBloquearComboAlmacen = function (event) {
		if (event) {
			if (self.TotalItems() > 0) {
				var textTitle =
					self.EstadoPendienteNota() == ESTADO_PENDIENTE_NOTA.PENDIENTE
						? "Para habilitar el almacén deberá borrar todo los ítems de la venta"
						: "";
	...
	self.CalcularTipoCambio = function (data, event) {
		if (event) {
			...
			if (self.IdMoneda() != ID_MONEDA_SOLES)
				if (
					data.ValorTipoCambio() != "0.00" &&
					data.ValorTipoCambio() != 0 &&
					jQuery.isNumeric(data.ValorTipoCambio())
				) {
		...
		if (event) {
			$(event.target).validate(function (valid, elem) {
				if (!valid) {
					...
					$(
						"#eac-container-" +
							$form.find("#input-teclado-virtual").val() +
							" ul"
					).hide();
				}
			});
		...
		if (event) {
			...
			if ($inputCliente.val().length != 11) {
				$inputCliente.attr(
					...
					"Ingrese el numero de documento correcto"
				);
			} else {
				$inputCliente.attr(
					"data-validation-error-msg",
					"No se han encontrado resultados para tu búsqueda de cliente"
				);
			}
			...
			if (data === -1) {
				...
				if (
					...
					if (data.NumeroDocumentoIdentidad == "") {
						...
						$inputCliente.attr(
							"data-validation-text-found",
							data.NumeroDocumentoIdentidad + " - " + data.RazonSocial
						);
					}
				}
				...
				if (
					data.CodigoDocumentoIdentidad == CODIGO_TIPO_DOCUMENTO_IDENTIDAD.RUC
				) {
				...
				if (self.DireccionesCliente().length > 0) {
					var direccionclientedefecto = self
				...
				if (self.ParametroAplicaPrecioEspecial() == "1") {
					$("#loader").show();
					self.AplicarPrecioEspecialCliente(
						data,
						event,
						function ($data, $event) {
							$("#loader").hide();
		...
		if (event) {
			...
			if (data === -1) {
				if (
					...
					var $evento = {
						target: self.Options.IDForm + " " + "#ClientePreCuenta",
					};
					self.ValidarCliente(data, $evento);
				} else {
					$inputCliente.attr("data-validation-text-found", "");
	...
	self.AplicarDescuentoProductoPorTarjeta = function (data, event) {
		if (event) {
			...
			if (objetocliente) {
				var indicadorafiliaciontarjeta =
						objetocliente.IndicadorAfiliacionTarjeta,
					fechainicioafiliaciontarjeta = new Date(
						objetocliente.FechaInicioAfiliacionTarjeta
					),
					fechafinafiliaciontarjeta = new Date(
						objetocliente.FechaFinAfiliacionTarjeta
					),
					clienteafiliado =
						indicadorafiliaciontarjeta == 1 &&
						hoy >= fechainicioafiliaciontarjeta &&
						hoy <= fechafinafiliaciontarjeta;
				...
				ko.utils.arrayForEach(self.DetallesComprobanteVenta(), function (item) {
					if (clienteafiliado && checkboxdescuento) {
						...
						if (objetoproducto.IdProducto) {
							var valordescuento = parseFloatAvanzado(
								objetoproducto.ValorDescuento
							);
							...
							if (objetoproducto.TipoDescuento == TIPO_DESCUENTO.MONTO) {
								...
								objetoproducto.TipoDescuento == TIPO_DESCUENTO.PORCENTUAL
							) {
					...
					var clientecontarjeta = clienteafiliado
						? "Afiliado a tarjeta 7"
						: "No afiliado a tarjeta 7";
	...
	self.ValidarAutoCompletadoRazonSocialDestinatario = function (data, event) {
		if (event) {
			$form
				.find("#RazonSocialDestinatario")
				.attr("data-validation-error-msg", "No se han encontrado resultados");
			...
			if (data === -1) {
				if (
					...
					var $evento = {
						target: self.Options.IDForm + " " + "#RazonSocialDestinatario",
					};
	...
	self.AplicarExcepcionValidaciones = function (data, event) {
		if (event) {
			...
			if (ultimoItem) {
				if (
					ultimoItem.CodigoMercaderia() === "" &&
					ultimoItem.NombreProducto() === "" &&
					(ultimoItem.Cantidad() === "" || ultimoItem.Cantidad() === "0") &&
					(ultimoItem.PrecioUnitario() === "" ||
						ultimoItem.PrecioUnitario() === "0") &&
					(ultimoItem.DescuentoUnitario() === "" ||
						ultimoItem.DescuentoUnitario() === "0")
				) {
					resultado = "true";
				}
				...
				$form
					...
					.attr("data-validation-optional", resultado);
				$form
					.find(ultimoItem.InputPrecioUnitarioSolesDolares())
					.attr("data-validation-optional", resultado);
				$form
					.find(ultimoItem.InputDescuentoUnitario())
					.attr("data-validation-optional", resultado);
				...
				$form
					...
					.attr("data-validation-optional", resultado);
				$form
					.find(ultimoItem.InputCantidadCaja())
					.attr("data-validation-optional", resultado);
				$form
					.find(ultimoItem.InputObservacion())
					.attr("data-validation-optional", resultado);
				$form
					.find(ultimoItem.InputDescuentoValorUnitario())
					.attr("data-validation-optional", resultado);
				$form
					.find(ultimoItem.InputValorUnitario())
					.attr("data-validation-optional", resultado);
				$form
					.find(ultimoItem.InputValorVentaItem())
					.attr("data-validation-optional", resultado);
			}
		}
	};
		...
		if (event) {
			...
			$.each(alumnos, function (key, entry) {
				if (data.IdCliente == entry.IdCliente) {
					...
					$form.find("#combo-alumno").append(
						$("<option " + sel + "></option>")
							.attr("value", entry.IdAlumno)
							.text(entry.NombreAlumno)
					);
				}
			});
		...
		if (event) {
			...
			$.each(cajas, function (key, entry) {
				if (self.IdMoneda() == entry.IdMoneda) {
					...
					$form.find("#combo-caja").append(
						$("<option " + sel + "></option>")
							.attr("value", entry.IdCaja)
							.text(entry.NombreCaja)
					);
				}
			});
		...
		if (event) {
			var montoTotal = parseFloatAvanzado(self.Total()),
				...
				vueltoRecibido =
					montoRecibido < 0
						? 0
						: parseFloatAvanzado(montoRecibido - montoTotal);
			...
			ko.utils.arrayForEach(self.DetallesComprobanteVenta(), function (item) {
				...
				if (
					...
					alertmsg +=
						"El " +
						numitem +
						"° ítem tiene una cantidad mayor al saldo actual del producto. <br>";
				}
			});
			return alertmsg;
		}
	...
	self.OnClickBtnTipoDocumentoVentaPreCuenta = function (data, event) {
		if (event) {
			var alertmsg = self.ValidarCantidadSegunSaldoPendientePreVenta(
				data,
				event
			);
			...
			if (self.IdCliente() != "" || self.IdCliente() != null) {
				...
				if (
					objetocliente.CodigoDocumentoIdentidad ==
						CODIGO_TIPO_DOCUMENTO_IDENTIDAD.RUC &&
					self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_FACTURA
				) {
					$form.find("#CheckCliente").closest(".form-group").hide();
					$form.find("#CheckCliente").prop("checked", true);
					self.CambiarClientesVarios(event);
					...
					self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_BOLETA ||
					self.IdTipoDocumento() == ID_TIPO_DOCUMENTO_TICKET
				) {
					$form.find("#CheckCliente").closest(".form-group").show();
					$form.find("#CheckCliente").prop("checked", true);
					self.CambiarClientesVarios(event);
		...
		if (event) {
			...
			ko.utils.arrayForEach(self.DetallesComprobanteVenta(), function (item) {
				...
				if (parseFloatAvanzado(item.SubTotal()) <= 0) {
					alertmsg +=
						"El " + numitem + "° ítem tiene una cantidad menor a 0. <br>";
				}
			});
			return alertmsg;
		}
	...
	self.OnHideOrShowElement = function (data, event) {
		if (event) {
			...
			if (self.DetallesComprobanteVenta().length <= 0) {
				alertify.alert(
					self.titulo,
					"Para realizar el pago tiene que seleccionar por lo menos un Producto.",
					function () {}
				);
				return false;
			}
	...
	self.OnClickBtnCargarNotaSalida = function (data, event) {
		if (event) {
			var datajs = {
				SerieNotaSalida: self.SerieNotaSalida(),
				NumeroNotaSalida: self.NumeroNotaSalida(),
			};
			$("#loader").show();
			self.ObtenerNotaSalidaVentaSinDocumento(
				datajs,
				event,
				function ($data, $event) {
					$("#loader").hide();
					if (!$data.error) {
						...
						if ($data.length <= 0) {
							alertify.alert(
								self.titulo,
								"No existe ninguna Nota de Salida " +
									self.SerieNotaSalida() +
									" - " +
									self.NumeroNotaSalida() +
									' con motivo "Comprobante sin Documento"',
								function () {}
							);
							return false;
						}
						...
						if (!ExisteAlgunSaldoPendienteComprobante) {
							alertify.alert(
								...
								"La Nota de Salida " +
									...
									" - " +
									self.NumeroNotaSalida() +
									" ya fue vinculada completamente con otras facturas",
								function () {}
							);
							return false;
						}
						...
						$data.forEach(function (entry, key) {
							...
							var producto = ObtenerJSONCodificadoDesdeURL(
								(ruta_producto =
									SERVER_URL + URL_RUTA_PRODUCTOS + entry.IdProducto + ".json")
							);
	...
	self.OnEnableDescuentoGlobal = ko.computed(function () {
		var cantidaditems = 0,
			cantidaditemGravado = 0,
			cantidaditemExonerado = 0,
			cantidaditemInafecto = 0;
		...
		if (
			cantidaditemGravado == cantidaditems ||
			cantidaditemExonerado == cantidaditems ||
			cantidaditemInafecto == cantidaditems
		) {
			return true;
		} else {
	...
	self.OnBuscarPersonaRENIEC = function (data, event) {
		if (event) {
			$("#loader").show();
			self.ConsultarNumeroDocumentoReniec(
				data,
				event,
				function ($data, $event) {
					$("#loader").hide();
	...
	self.OnChangeOrigenDestino = function (data, event) {
		if (event) {
			if (
				self.IdLugarOrigen() != undefined &&
				self.IdLugarDestino() != undefined
			) {
				$form.resetearValidaciones();
				...
				var nombreProducto =
					`SERVICIO DE ${nombreOrigen} A ${nombreDestino} ${self.Observacion()}`.trim();
				var detalle = self.DetallesComprobanteVenta()[0];

				var rpta = self.ObtenerServicioPorNombreProducto(
					{ NombreProducto: nombreProducto },
					event
				);

				if (rpta.length > 0) {
					var servicio = ObtenerJSONCodificadoDesdeURL(
						SERVER_URL + URL_RUTA_PRODUCTOS + rpta[0].IdProducto + ".json"
					);
	...
	self.ObtenerComprobantesVentaReferencia = function (data, event) {
		if (event) {
			if (
				self.ParametroRubroClinica() != 1 ||
				self.IdTipoDocumento() != ID_TIPO_DOCUMENTO_ORDEN_PEDIDO
			) {
				return false;
			}
			if (ViewModels.data.ComprobantesVentaReferenciaClinica().length > 0) {
				return false;
			}

			$("#loader").show();
			self.ConsultarComprobantesVentaReferencia(
				data,
				event,
				function ($data, $event) {
					$("#loader").hide();
					if (!$data.error) {
						ViewModels.data.ComprobantesVentaReferenciaClinica(
							Knockout.CopiarObjeto($data)()
						);
						var objeto = { id: "#DocumentoVentaReferencia", data: $data };
						$form
							.find(objeto.id)
							.autoCompletadoComprobanteVentaReferencia(
								objeto,
								event,
								self.ValidarAutoCompletadoComprobanteVentaReferencia
							);
					} else {
						alertify.alert(self.titulo, $data.error.msg, function () {});
					}
	...
	self.ValidarAutoCompletadoComprobanteVentaReferencia = function (
		data,
		event
	) {
		if (event) {
			...
			var $evento = {
				target: `${self.Options.IDForm} #DocumentoVentaReferencia`,
			};
			...
			if (data === -1) {
				...
				var $data = {
					...
					IdComprobanteVentaReferencia: data.IdComprobanteVenta,
				};
			}

			$inputComprobanteReferencia.attr(
				"data-validation-error-msg",
				memsajeError
			);
			$inputComprobanteReferencia.attr(
	...
	self.ValidarAutoCompletadoRadioTaxi = function (data, event) {
		if (event) {
			...
			if (data === -1) {
				...
				var $data = {
					Documento: data.NombreRadioTaxi,
	...
	self.OnChangeComboVendedores = function (data, event) {
		if (event) {
			if (self.ComprobanteVentaInicial) {
				self.ComprobanteVentaInicial.AliasUsuarioVenta =
					self.AliasUsuarioVenta();
				if (self.opcionProceso() == opcionProceso.Nuevo) {
					ViewModels.data.ComprobanteVentaNuevo.AliasUsuarioVenta(
						self.AliasUsuarioVenta()
					);
				}
			}
	...
	self.ObtenerComprobantesVentaProforma = function (data, event) {
		if (event) {
			if (self.ParametroProforma() != 1) {
				return false;
			}
			if (ViewModels.data.ComprobantesVentaProforma().length > 0) {
				return false;
			}

			$("#loader").show();
			self.ConsultarComprobantesVentaProforma(
				data,
				event,
				function ($data, $event) {
					$("#loader").hide();
					if (!$data.error) {
						ViewModels.data.ComprobantesVentaProforma(
							Knockout.CopiarObjeto($data)()
						);
	...
	self.ValidarAutoCompletadoComprobanteVentaProforma = function (data, event) {
		if (event) {
			var $inputComprobanteReferenciaProforma = $form.find(
				"#DocumentoVentaProforma"
			);
			var $evento = {
				target: `${self.Options.IDForm} #DocumentoVentaProforma`,
			};
			...
			if ($inputComprobanteReferenciaProforma.val() !== "") {
				if (data === -1) {
					...
					var $data = {
						Documento: data.Documento,
						IdReferenciaProforma: data.IdComprobanteVenta,
					};
					var comprobante = data;
				}

				$inputComprobanteReferenciaProforma.attr(
					"data-validation-error-msg",
					memsajeError
				);
				$inputComprobanteReferenciaProforma.attr(
			...
			var pos = $inputComprobanteReferenciaProforma
				.closest("Form")
				.find("input, select")
				.not(":disabled")
				.index($inputComprobanteReferenciaProforma);
			$inputComprobanteReferenciaProforma
				.closest("Form")
				.find("input, select")
				.not(":disabled")
				.eq(pos + 1)
	...
	self.OnPorcentajeComision = function (data, event) {
		if (event) {
			...
			var montocomision = parseFloatAvanzado(
				(porcentajecomision * self.Total()) / 100
			);
			self.MontoComision(
				parseFloatAvanzado(montocomision.toFixed(NUMERO_DECIMALES_VENTA))
			);
		}
	};
		...
		if (event) {
			...
			if (data === -1) {
				...
				var $data = {
					...
					KilometrajeVehiculo: data.UltimoKilometraje,
				};
			}
			...
			var RadioTaxis = ObtenerJSONCodificadoDesdeURL(
				SERVER_URL + URL_JSON_RADIO_TAXI
			);
			var RadioTaxi = JSON.search(
				RadioTaxis,
				'//*[ IdRadioTaxi ="' + data.IdRadioTaxiActual + '"]'
			);
	...
	self.OnClickBtnCasillerosPorGenero = function (data, event) {
		if (event) {
			...
			if (
				casillerosPorGenero.Masculino().length > 0 &&
				casillerosPorGenero.Femenino().length > 0
			) {
				...
				self.ListarCasillerosGenero(datajs, event, function ($data) {
					$("#loader").hide();
					if (!$data.error) {
						var casillerosMasculinos = Knockout.CopiarObjeto(
							$data.filter((item) => item.IdGenero == ID_GENERO_MASCULINO)
						);
						var casillerosFemeninos = Knockout.CopiarObjeto(
							$data.filter((item) => item.IdGenero == ID_GENERO_FEMENINO)
						);
						...
						if (self.opcionProceso() == opcionProceso.Edicion) {
							var casilleros =
								self.IdGenero() == ID_GENERO_MASCULINO
									? casillerosMasculinos
									: casillerosFemeninos;
							var casillero = ko.utils.arrayFirst(
								casilleros(),
								(item) => item.IdCasillero() == self.IdCasillero()
							);
	...
	self.OnHideCasillerosPorGenero = function (event) {
		if (event) {
			var existeCasilleroSeleccionado = self.CasilleroSeleccionado.IdCasillero
				? true
				: false;

			var data = {
				IdCasillero: existeCasilleroSeleccionado
					? self.CasilleroSeleccionado.IdCasillero()
					: "",
				NombreCasillero: existeCasilleroSeleccionado
					? self.CasilleroSeleccionado.NombreCasillero()
					: "",
				IdGenero: existeCasilleroSeleccionado
					? self.CasilleroSeleccionado.IdGenero()
					: "",
				NombreGenero: existeCasilleroSeleccionado
					? self.CasilleroSeleccionado.NombreGenero()
					: "",
			};
	...
	self.CambiosEnElDetale = ko.computed(function () {
		...
		ko.utils.arrayForEach(detalles, function (item, key) {
			...
			if (item.IdProducto() !== null && item.IdProducto() !== "") {
				totalCantidad =
					parseFloatAvanzado(totalCantidad) +
					parseFloatAvanzado(item.Cantidad());
			}
		});
	...
	self.OnChangeMontoRecibido = function (data, event) {
		if (event) {
			if (self.ParametroCampoACuenta() == 1) {
				var debe =
					parseFloatAvanzado(self.Total()) -
					parseFloatAvanzado(self.MontoACuenta());
				var vuelto =
					parseFloatAvanzado(self.MontoRecibido()) - parseFloatAvanzado(debe);
			} else {
				var debe = parseFloatAvanzado(self.MontoDebe());
				var vuelto =
					parseFloatAvanzado(self.MontoRecibido()) -
					parseFloatAvanzado(self.Total());
			}
	...
	self.OnClickBtnNuevoVehiculoCliente = function (data, event, cliente) {
		if (event) {
			if (
				...
				alertify.alert(
					self.titulo,
					"Debe seleccionar un cliente para agregar nuevas placas",
					function () {}
				);
				return false;
			}
	...
	self.CalcularSaldo = ko.computed(function () {
		var total = accounting.formatNumber(self.Total(), NUMERO_DECIMALES_VENTA);
		var montoACuenta = accounting.formatNumber(
			self.MontoACuenta(),
			NUMERO_DECIMALES_VENTA
		);
		var resultado = total - montoACuenta;
		return accounting.formatNumber(resultado, NUMERO_DECIMALES_VENTA);
	}, this);
	...
	self.HideCuotasPago = function (event) {
		if (event) {
	...
	self.CalcularTotalCuotaPago = function(data,event) {
		if(event) {
	...
	self.OnClickEstadoDetraccion = function(data,event) {
		if(event) {
			if (event.target.id) {				
				var idcheckbox ="CheckEstadoDetraccion";
				var checkedEstadoRegularizacionProducto =data.EstadoDetraccion();
          if (checkedEstadoRegularizacionProducto == false) {
            self.EstadoDetraccion(true);
	...
	self.ValidarPorcentajeDetraccion = function (data, event) {
    if (event) {           
      if (data.PorcentajeDetraccion() === "") data.PorcentajeDetraccion("0.00");
      $(event.target).validate(function (valid, elem) {
      });      
    }
  }

self.ActualizarResumenCuotasPago = function(data,event) {
		if(event) {
			//debugger;
			console.log("ActualizarResumenCuotasPago");
			if (self.CuotasPagoClienteComprobanteVenta != undefined) {
				var detalles = self.CuotasPagoClienteComprobanteVenta();
			} else {
				var detalles = [];
			...
			if (detalles.length > 0) {
				var saltolinea ="\n";
				var lineatitulo ="DATOS DE CUOTA:"+saltolinea;
				var lineacabecera ="N° CUOTA\t\t MONTO\t\t FECHA VCTO"+saltolinea;
				var observacionResumenCuotas =  lineatitulo + lineacabecera;//(detalles.length == 0 ? "" : "Cuotas de Pago"+saltolinea);
				
				//moment.locale('es');				
				var fecha1 = moment(Date(self.FechaEmision()));
				var fecha1formateado =moment(fecha1.format("YYYY-MM-DD"));
				var UltimaFechaCuotaPago;

				ko.utils.arrayForEach(detalles, function (item, key) {			
					var fecha2 = moment(Date(item.FechaPagoCuota()));
					var fecha2formateado =moment(fecha2.format("YYYY-MM-DD"));					
					var dias = fecha2formateado.diff(fecha1formateado, 'days');
					linea=item.NumeroCuota()+"\t\t"+item.MontoCuota()+"\t\t"+item.FechaPagoCuota()+saltolinea;//observacionResumenCuotas = observacionResumenCuotas+ "Cuota " + item.NumeroCuota()+" A "+dias+" DIAS ("+item.FechaPagoCuota()+") : S/."+ item.MontoCuota()+saltolinea;
					observacionResumenCuotas = observacionResumenCuotas + linea;					
					UltimaFechaCuotaPago =item.FechaPagoCuota();
					item.IdMoneda(self.IdMoneda());
				});
				
				self.ResumenCuotasPagoFormaPagoCredito(observacionResumenCuotas);	
				self.FechaVencimiento(UltimaFechaCuotaPago);
			}
		}
	}

	self.OnChangeCheckEstadoRetencionIGV = function(data,event) {
		if(event) {
				var checked = $form.find("#CheckEstadoRetencionIGV").prop("checked");
				if (checked) {
					self.EstadoRetencionIGV(true);	
					self.PorcentajeRetencionIGV(self.ParametroPorcentajeRetencionIGV());	
					$form.find("#PorcentajeRetencionIGV").attr("readonly", false);
				} else {
					self.EstadoRetencionIGV(false);					
					self.PorcentajeRetencionIGV("0.00");	
					$form.find("#PorcentajeRetencionIGV").attr("readonly", true);
				}
		}	
	}

	self.OnClickEstadoRetencionIGV = function(data,event) {
		if(event) {
			if (event.target.id) {				
				var idcheckbox ="CheckEstadoRetencionIGV";
				var checked =data.EstadoRetencionIGV();
          if (checked == false) {
            self.EstadoRetencionIGV(true);
            $("#"+idcheckbox).prop("checked",true);
          }
	...
	self.CalculoMontoRetencionIGV = ko.computed(function () {
		var porcentajeRetencionIGV=parseFloat(self.PorcentajeRetencionIGV() == "" ? 0 : self.PorcentajeRetencionIGV());
		var total = parseFloat(self.Total());
		var resultado = accounting.formatNumber(porcentajeRetencionIGV*total/100, NUMERO_DECIMALES_VENTA);
		self.MontoRetencionIGV(resultado);
		self.BaseImponibleRetencionIGV(total);
		return resultado;
	}, this);

	self.ValidarPorcentajeRetencionIGV = function (data, event) {
    if (event) {           
      if (data.PorcentajeRetencionIGV() === "") data.PorcentajeRetencionIGV("0.00");
      $(event.target).validate(function (valid, elem) {
      });      
    }
  }

	self.CalculoMontoAPagar = ko.computed(function () {
		
			var total = parseFloat(self.Total());
			var _montoretencionigv= self.MontoRetencionIGV().toString().replace(",", "");
			var _detraccion = self.MontoDetraccion().toString().replace(",", "");
			var retencion =  parseFloat(_montoretencionigv);
			var detraccion =  parseFloat(_detraccion);
			var montoAPagar = total - retencion - detraccion;
			var resultado = accounting.formatNumber(montoAPagar,NUMERO_DECIMALES_VENTA);
