...
VistaModeloDetalleCompraCostoAgregado = function (data) {
  var self = this;
  ko.mapping.fromJS(data, MappingCatalogo, self);
  ModeloDetalleCompraCostoAgregado.call(this, self);
  self.DecimalCantidad = ko.observable(CANTIDAD_DECIMALES_COSTO_AGREGADO.CANTIDAD);
  self.DecimalCostoUnitario = ko.observable(CANTIDAD_DECIMALES_COSTO_AGREGADO.COSTO_UNITARIO);
  self.DecimalPrecioUnitario = ko.observable(CANTIDAD_DECIMALES_COSTO_AGREGADO.PRECIO_UNITARIO);
  ...
  self.InicializarVistaModelo = function (event) {
    if (event) {
      ...
      $(self.InputProducto()).autoCompletadoCostoAgregado(input, event, self.ValidarAutocompletadoProducto);
      ...
      // $(self.InputAfectoIGV()).prop("disabled", true);
    }
  }
  ...
  self.ValidarProductoPorIdProducto = function (data, event, $callback) {
    if (event) {
      ...
      var datajs = { IdProducto: data.IdProducto(), IdGrupoProducto: TIPO_COMPRA.COSTOSAGREGADO };
