ModeloSaldoInicialCuentaCobranza = function (data) {
    var self = this;
    var base = data;

    self.GuardarSaldoInicialCuentaCobranza = function (data, event, callback) {
        if(event) {
          var objeto = ko.mapping.toJS(data, mappingIgnore);

          if (data.OpcionProceso() == opcionProceso.Nuevo) {
            objeto.IdSaldoInicialCuentaCobranza = "";
    ...
    self.EliminarSaldoInicialCuentaCobranza = function (data, event, callback) {
        if(event) {
    ...
    self.Insertar = function(data,event,callback) {
      if (event) {
        $.ajax({
          ...
          data : data,
          dataType: "json",
          url: SITE_URL+'/CuentaCobranza/cSaldoInicialCuentaCobranza/InsertarSaldoInicialCuentaCobranza',
    ...
    self.Actualizar = function(data,event,callback) {
      if (event) {
        $.ajax({
          ...
          data : data,
          dataType: "json",
          url: SITE_URL+'/CuentaCobranza/cSaldoInicialCuentaCobranza/ActualizarSaldoInicialCuentaCobranza',
          success: function (data) {
            callback(data,event);
          },
          error :  function (jqXHR, textStatus, errorThrown) {
    ...
    self.Eliminar = function(data,event,callback) {
      if (event) {
        $.ajax({
          ...
          data : data,
          dataType: "json",
          url: SITE_URL+'/CuentaCobranza/cSaldoInicialCuentaCobranza/BorrarSaldoInicialCuentaCobranza',
          success: function (data) {
            callback(data,event);
          },
          error :  function (jqXHR, textStatus, errorThrown) {
