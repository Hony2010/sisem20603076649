VistaModeloConsultaCuentaCobranza = function (data) {

    var self = this;
    ko.mapping.fromJS(data, MappingCuentaCobranza, self);
    ModeloConsultaCuentaCobranza.call(this,self);

    self.Inicializar = function ()  {
      if(self.data.CuentasCobranza().length > 0)
      {
        var objeto = self.data.CuentasCobranza()[0];
    ...
    self.PostAnular = function (data,event) {
      if(event) {
        $("#loader").hide();
        if(!data.error) {
          var objeto = ko.utils.arrayFirst(self.data.CuentasCobranza(),function (item) {return item.IdComprobanteCaja() == data.IdComprobanteCaja;});
    ...
    self.PostEliminar = function (data,event) {
      if(event) {
        if(!data.error) {
          ...
          var objeto = ko.utils.arrayFirst(self.data.CuentasCobranza(),function (item) {return item.IdComprobanteCaja() == data.IdComprobanteCaja;});
          self.data.CuentasCobranza.remove(objeto);

          // var filas = self.data.CuentasCobranza().length;
    ...
    self.PostGuardar = function(data,event) {
      if(event) {
        if(data) {
          ...
          self.data.CuentasCobranza.replace(item, objeto);
    ...
    self.ConsultarPorPagina = function (data,event) {
      if(event) {
          self.ListarCuentasCobranzaPorPagina(data,event,self.PostConsultarPorPagina);
      }
    }
    ...
    self.Consultar = function (data,event) {
      if(event) {
        ...
        if(tecla == TECLA_ENTER)
        {
          ...
          self.ListarCuentasCobranza(data,event,self.PostConsultar);
        }
      }
    }
